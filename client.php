...
$update = "UPDATE orders SET status ='Delivered' WHERE ticket='".$ticket."' ";
...
<th>Status</th>
  ...
  $get_directory= mysql_query("SELECT * FROM ".$userid."_orders ORDER BY ID DESC");
...
<td>
    ...
    <input type="hidden" name="ticket" id="1" class="form-control" value="<?php echo $ticket;?>" required="" autofocus="">
      ...
      <div style="height: 10px;"> </div>
        <button class="btn btn-sm btn-primary btn-block"  <?php if ($status == "Pending" or $status == "Ready") {
         echo "";?>disabled="disabled"<?php
        }?> type="submit" name="update">Delivered</button>
  <div style="height: 10px;"> </div>
